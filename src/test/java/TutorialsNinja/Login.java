...
//		System.out.println(loginpage.checkWarning());
	...
	@Test(priority=6)
	public void verifyForgotPasswordLinkTextIsWorkingOrNot()
	{
		
		loginpage.clickLogin();
		
		if(loginpage.ForgottenPassisDisplay())
		{
			System.out.println("Test passed. Forgotten password link text is present.");
			loginpage.ClickOnForgottenPass();
			String expected = "Forgot Your Password?";
			Assert.assertEquals(expected, loginpage.ForgottenPassPageContent());
			System.out.println(loginpage.ForgottenPassPageContent());
			if(expected.contains(loginpage.ForgottenPassPageContent()))
			{
				System.out.println("Test Pass. Forgotten password page is displayed.");
			}
			else
			{
				System.out.println("Test failed. Forgotten password page is not displayed.");
			}
		}
		else
		{
			System.out.println("Test failed. Forgotten password link text is not present.");
		}
